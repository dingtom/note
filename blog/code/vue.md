...
- [ el:挂载点](#head2)
- [ data:数据对象](#head3)
- [ v-text:设置标签的文本值](#head4)
- [ v-html:设置标签的innerHTML](#head5)
- [ v-on:为元素绑定事件](#head6)
	- [ [事件修饰符]](#head7)
- [ v-show:根据表达值的真假,切换元素的显示和隐藏](#head8)
- [ v-if:根据表达值的真假,切换元素的显示和隐藏(操纵dom元素)](#head9)
- [ v-bind设置元素的属性(如:src,title,class)](#head10)
	- [ v-bind绑定class](#head11)
	- [ v-bind绑定style](#head12)
- [ v-for:根据数据生成列表结构](#head13)
	- [ 组件的key属性](#head14)
- [  v-model:获取和设置表单元素的值(双向数据绑定)](#head15)
	- [ 修饰符](#head16)
- [ v-once:元素和组件(组件后面才会学习)只渲染一次](#head17)
- [  笔记本应用](#head18)
- [ 计算属性](#head19)
- [ 过滤器](#head20)
- [ vue-router](#head21)
	- [ URL的hash](#head22)
	- [ HTML5的history模式：pushState](#head23)
		- [history.pushState(data, title, url)      ](#head24)
		- [ history.replaceState()](#head25)
		- [ history.go()](#head26)
	- [ vue-router](#head27)
	- [ 例子](#head28)
	- [ router-link/view组件](#head29)
	- [ linkActiveClass](#head30)
	- [ 路由代码跳转](#head31)
	- [ 动态路由](#head32)
	- [ 懒加载](#head33)
	- [ \$router和\$route](#head34)
	- [ 嵌套路由](#head35)
		- [ 嵌套默认路径](#head36)
	- [ 传递参数](#head37)
		- [ params的类型:](#head38)
		- [ query的类型:](#head39)
	- [ 导航守卫](#head40)
	- [ keep-alive遇见vue-router](#head41)
- [ 父组件和子组件](#head42)
	- [ 父子组件的通信](#head43)
	- [ props父组件向子组件传递数据](#head44)
	- [ 子级向父级传递](#head45)
- [ 父子组件的访问方式](#head46)
- [ \$children](#head47)
- [ $refs](#head48)
...
# <span id="head3"> data:数据对象</span>

Vue中用到的数据定义在data中
data中可以写复杂类型的数据
渲染复杂类型数据时遵守is的语法即可

# <span id="head4"> v-text:设置标签的文本值</span>
...
# <span id="head5"> v-html:设置标签的innerHTML</span>
...
# <span id="head6"> v-on:为元素绑定事件</span>
...
# <span id="head8"> v-show:根据表达值的真假,切换元素的显示和隐藏</span>
...
# <span id="head9"> v-if:根据表达值的真假,切换元素的显示和隐藏(操纵dom元素)</span>
...
# <span id="head10"> v-bind设置元素的属性(如:src,title,class)</span>

完整写法是 v-bind:属性名, 简写的话可以直接省略v-bind，**只保留 :属性名**

### <span id="head11"> v-bind绑定class</span>
...
### <span id="head12"> v-bind绑定style</span>
...
# <span id="head13"> v-for:根据数据生成列表结构</span>
...
# <span id="head15">  v-model:获取和设置表单元素的值(双向数据绑定)</span>
...
# <span id="head17"> v-once:元素和组件(组件后面才会学习)只渲染一次</span>

该指令后面不需要跟任何表达式(比如之前的v-for后面是由跟表达式的)

该指令表示元素和组件(组件后面才会学习)只渲染一次，不会随着数据的改变而改变。

# <span id="head18">  笔记本应用</span>
...
# <span id="head21"> vue-router</span>

改变URL，但是页面不进行整体的刷新。

## <span id="head22"> URL的hash</span>
...
## <span id="head23"> HTML5的history模式：pushState</span>

history接口是HTML5新增的, 它有五种模式改变URL而不刷新页面.

### <span id="head24">history.pushState(data, title, url)      </span>
...
## <span id="head27"> vue-router</span>
...
## <span id="head29"> router-link/view组件</span>
...
## <span id="head30"> linkActiveClass</span>
...
## <span id="head34"> \$router和\$route</span>
...
## <span id="head41"> keep-alive遇见vue-router</span>
...
### <span id="head44"> props父组件向子组件传递数据</span>
...
## <span id="head46"> 父子组件的访问方式</span>

### <span id="head47"> \$children</span>
...
## <span id="head59"> Promise三种状态</span>
...
## <span id="head60"> Promise链式调用</span>
...
## <span id="head61">async \ await</span>
...
## <span id="head65"> lessloader</span>
