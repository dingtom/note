...
- [ C4.5](#head14)
	...
	- [ 缺点](#head22)
- [CART(Classification And Regression Tree)](#head23)
	- [ 思想](#head24)
	- [ 剪枝策略](#head25)
	- [ 类别不平衡](#head26)
	- [ CART回归树](#head27)
...
信息熵**表示了信息的不确定度，理解起来就是衡量一组样本的混乱程度**，样本越混乱，越不容易做出决定。
...
得知特征 A 的信息而使得样本集合不确定性减少的程度。计算公式如下：
...
### <span id="head21"> 问题二：选定该划分特征，对于缺失该特征值的样本如何处理？（即到底把这个样本划分到哪个结点里）</span>
...
# <span id="head23">CART(Classification And Regression Tree)</span>
...
#### <span id="head36"> 预测方式</span>

对于决策树建立后做预测的方式，上面讲到了 CART 分类树采用叶子节点里概率最大的类别作为当前节点的预测类别。而回归树输出不是类别，它采用的是用最终叶子的均值或者中位数来预测输出结果。

# <span id="head37"> 总结</span>
