...
5.  列抽样，传统的GBDT在每轮迭代时使用全部的数据，XGBoost 借鉴了随机森林的做法，**支持列抽样，不仅能降低过拟合，还能减少计算；**
6.  缺失值处理，传统的GBDT没有设计对缺失值进行处理，XGBoost 采用的**稀疏感知算法极大的加快了节点分裂的速度；**
7.  **可以并行**化操作，块结构可以很好的支持并行计算。
8.  非线性变换比较多，表达能力强，而且不需要做复杂的特征工程和特征变换。
...
# <span id="head16"> 代码</span>

## <span id="head17"> sklearn</span>
